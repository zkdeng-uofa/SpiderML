...
class ViTMAEForImageClassification(torch.nn.Module):
    def __init__(self, model_name, num_labels):
        ...
        self.vit_mae = ViTMAEModel.from_pretrained(model_name)
        self.classifier = torch.nn.Linear(self.vit_mae.config.hidden_size, num_labels)
    
    def forward(self, pixel_values, labels=None):
        outputs = self.vit_mae(pixel_values=pixel_values)
        sequence_output = outputs.last_hidden_state[:, 0]  # take the cls token output
...
def main(hubPath, hubModel):
    ...
    model = ViTMAEForImageClassification(model_checkpoint, num_labels)
    model.to(device)
...
if __name__ == "__main__":
    set_seed(42)
    #args = parse_HF_args()
    main("zkdeng/dangerousSpiders", "facebook/vit-mae-large")
